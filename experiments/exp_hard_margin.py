...
def plot_all_boundaries(step_x=6, step_y=6, n_grid=100):
    ...
    fig = plotly.subplots.make_subplots(rows=1, cols=2, shared_yaxes=True)
    ...
    landscape_max += 1 - landscape_max.min()
    landscape_max = np.log10(landscape_max)

    palette = plotly.colors.diverging.RdBu_r
    colorscale = palette
    # colorscale_split = 1 / (np.max(landscape_max) - np.min(landscape_max))
    # colorscale_lower = list(np.linspace(0, colorscale_split,
    #                                     num=len(palette) // 2, endpoint=False))
    # colorscale_upper = list(np.linspace(colorscale_split, 1,
    #                                     num=1 + len(palette) // 2))
